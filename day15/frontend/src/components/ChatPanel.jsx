    ...
    <section className="chat">
      ...
      <div className="conversation" ref={conversationRef}>
        ...
        {messages.map((item, index) => (
          ...
          >
            <div
              className={`bubble ${item.role === 'user' ? '' : 'markdown'}${
                item.violation ? ' bubble--violation' : ''
              }`}
            >
              {item.role === 'user'
                ? item.content
            ...
            {item.role === 'assistant' && item.violation && (
              <div className="violation">
                <span className="violation-chip mono">
                  запрос отклонён
                </span>
                <span className="violation-text">
                  {item.violation.invariant
                    ? `нарушен инвариант «${item.violation.invariant.text}» (${item.violation.invariant.category_label})`
                    : 'нарушен установленный инвариант'}
                </span>
              </div>
            )}
