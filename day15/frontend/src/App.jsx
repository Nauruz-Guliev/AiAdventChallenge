...
export default function App() {
    ...
    try {
      ...
      setMessages(current => [
        ...current,
        {
          ...
          violation: response.violation ?? null,
        },
      ]);
      setResult(response);
  ...
  async function handleApproveTask() {
    try {
      setTaskState(await applyTaskTransition('approve_plan'));
    } catch (requestError) {
      setError(requestError.message);
      setTaskState(await getTaskState());
    }
  ...
  async function handleAddInvariant(payload) {
    try {
      await createInvariant(payload);
      await refreshInvariants();
      return true;
    } catch (requestError) {
      setError(requestError.message);
  ...
  async function handleDeleteInvariant(invariantId) {
    try {
      await deleteInvariant(invariantId);
      await refreshInvariants();
    } catch (requestError) {
      setError(requestError.message);
    }
  ...
  return (
    <main className="app">
      <header className="masthead">
        <div>
          <h1>Контролируемые переходы состояний</h1>
          <p>
            У задачи есть явные состояния и таблица разрешённых переходов:
            планирование → утверждение плана → выполнение → проверка → готово.
            Ассистент не может перепрыгнуть этап — попытка недопустимого
            перехода отклоняется и фиксируется.
          </p>
        </div>
      </header>
      ...
      <InvariantsPanel
        ...
        onAdd={handleAddInvariant}
        onDelete={handleDeleteInvariant}
      />
